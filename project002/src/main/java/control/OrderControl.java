...
public class OrderControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<OrderProduct> orderlist = order.getOrder(info.getId());
        request.setAttribute("orderlist", orderlist);

        List<UserCart> cartlist = order.getUserCart(us.getUsername());
        request.setAttribute("cartlist", cartlist);

        HashMap<UserCart, ProductFullInfo> productmap = new HashMap<UserCart, ProductFullInfo>();

        List<ProductFullInfo> productlist = order.getAllProductFullInfo();
        ...
        User info1 = order.getUserInfo(info.getId());
