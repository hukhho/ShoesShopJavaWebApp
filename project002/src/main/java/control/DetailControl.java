...
public class DetailControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int count = 0;
        for (ProductFullInfo x : listfullbypid) {
            count = x.getAmount();
        }
        if (count == 0) {
              request.setAttribute("errorInventory", "Sold out");
        }
        if (listfullbypid.isEmpty()) {
            request.setAttribute("errorInventory", "This product not avaiable now!");
        }
